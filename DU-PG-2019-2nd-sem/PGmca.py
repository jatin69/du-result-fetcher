...
for col in all_colleges:
    ...
    for VAR_stud in dduc:
        ...
        if(sgpa_table == None ):
            continue
        try:
            obtained_marks = int(sgpa_table.findAll("td")[5].text)
            FINAL_CGPA = float(obtained_marks/5.0)
            # FINAL_CGPA = float(total.contents[5].contents[4].contents[0])
...
# print to file
with open('DU-PG-MCA-2019-2nd-sem.txt','w') as f:
...
# CSV print to file
with open('DU-PG-MCA-2019-2nd-sem.csv.txt','w') as f:
    print('{3:<5} ,{0:15} ,{1:25} ,{2:10} ,{4:<40}'.format("Roll No.","Name","SGPA","S.No", "College"), file=f)
    for i,marks in enumerate(college_sgpa_list):
        print('{3:<5} ,{0:15} ,{1:25} ,{2:<10} ,{4:<40}'.format(marks[0],marks[1],marks[2], i+1, marks[3]), file=f)
