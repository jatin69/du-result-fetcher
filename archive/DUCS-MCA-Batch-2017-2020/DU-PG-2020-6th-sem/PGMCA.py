...
for col in all_colleges:
    ...
    for VAR_stud in dduc:
        ...
        # GET 6th sem marks
        try:
            obtained_marks = int(sgpa_table.findAll('tr')[6].findAll('td')[1].text)
            total_marks = 5
        ...
        try:
            ...
            # print("obtained is : {}", obtained_marks)
...
with open('DU-PG-MCA-2017-2020-aggregate.txt','w') as f:
    ...
    for i,marks in enumerate(college_aggregate_sgpa_list):
        print('{3:<5} {0:15} {1:25} {2:<10} {4:<40}'.format(marks[0],marks[1], str(round(marks[2], 3)), i+1, marks[3]), file=f)
