...
for col in all_colleges:
    ...
    for VAR_stud in dduc:
        ...
        total = soup.find("span", id="lbl_gr_cgpa")
        ...
        if(total == None ):
            continue
        try:
            FINAL_CGPA = float(total.text.replace(' ',':').replace(',',':').split(':')[4])
...
# print to file
with open('DU-CSHONS-2016-2019-AGGREGATE-Result.txt','w') as f:
    print('{3:<5} {0:15} {1:25} {2:10} {4:<40}'.format("Roll No.","Name","CGPA","S.No", "College"), file=f)
...
# CSV print to file
with open('DU-CSHONS-2016-2019-AGGREGATE-Result.csv.txt','w') as f:
    print('{3:<5} ,{0:15} ,{1:25} ,{2:10} ,{4:<40}'.format("Roll No.","Name","CGPA","S.No", "College"), file=f)
    for i,marks in enumerate(college_sgpa_list):
        print('{3:<5} ,{0:15} ,{1:25} ,{2:<10} ,{4:<40}'.format(marks[0],marks[1],marks[2], i+1, marks[3]), file=f)
